...
object PinyinUtil {
    private val sPinYinValue = intArrayOf(
        -20319,
        -20317,
        -20304,
        -20295,
    ...
    private val sPinYinStr = arrayOf(
        "a",
        "ai",
        "an",
        "ang",
     ...
     * 单个汉字转成ASCII码
     */
    private fun getAscii(chs: String): Int {
        var asc = 0
        try {
            ...
            println("ERROR:ChineseSpelling.class-char2Ascii(String chs)$e")
        }
        return asc
    }
     ...
     * 单个汉字转换成拼音
     */
    private fun getSinglePinYin(str: String): String? {
        var result: String? = null
        val ascii = getAscii(str)
        if (ascii in 1..159) {
            result = (ascii.toChar()).toString()
        } else {
            for (i in sPinYinValue.size - 1 downTo 0) {
    ...
    fun getPinYinList(source: String?): List<String?>? {
        ...
        for (i in 0 until source.length) {
            ...
            if (item.toByteArray().size >= 2) {
                var pinyin = getSinglePinYin(item)
                if (pinyin == null) {
                    pinyin = item
                }
